...
class ZipDataIngestor(DataIngestor):
    def ingest(self, file_path: str) -> pd.DataFrame:
        ...
        # Find the 'data' folder inside 'extracted_data'
        data_folder = os.path.join("extracted_data", "data")
        
        if not os.path.exists(data_folder):
            raise FileNotFoundError("The 'data' subfolder was not found in the extracted contents.")
        
        # List the CSV files in the 'data' folder
        csv_files = [f for f in os.listdir(data_folder) if f.endswith(".csv")]

        if len(csv_files) == 0:
            raise FileNotFoundError("No CSV file found in the 'data' folder.")
        
        # Assuming the second CSV is the one you want
...
#     file_path = "/Users/kishenkumarsivalingam/Documents/MLE Assessment/data.zip"
